...
const exeName = process.platform === "win32" ? "dart-decimate-mcp.exe" : "dart-decimate-mcp";
...
run(cargo, ["run", "--release", "--locked", "--bin", "dart-decimate-mcp", "--", ...args], root);
...
function run(command, commandArgs, cwd = undefined) {
  ...
  if (result.error) {
    if (result.error.code === "ENOENT") {
      console.error(
        "dart-decimate-mcp: Rust/Cargo is required to build the npm source package. " +
          "Install Rust from https://rustup.rs or use a release package with a prebuilt binary."
      );
      process.exit(127);
    }
    console.error(`dart-decimate-mcp: failed to execute ${command}: ${result.error.message}`);
    process.exit(1);
  }
